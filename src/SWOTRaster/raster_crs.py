...
def mgrs_band_shift(mgrs_band, shift, longitude):
    ...
    if band_num < MGRS_VALID_BANDS.find('C'):
        if longitude < 0:
            return 'A'
        return 'B'
    if band_num > MGRS_VALID_BANDS.find('X'):
        if longitude < 0:
            return 'Y'
        return 'Z'
...
def utm_crs_from_polygon(poly, utm_zone_adjust=0, mgrs_band_adjust=0):
    """ Gets the UTM Coordinate Reference System at the centroid of a
    polygon """
...
def split_wrapped_longitude_polygon(polygon, thresh=180):
    ...
    shifted_polygon = shift_wrapped_longitude_polygon(polygon, thresh)
...
def llh2bearing(llh0, llh1):
    """ Returns the bearing from one set of LLH coordinates to another set of
    LLH coordinates """
...
def terminal_loc_spherical(latitude, longitude, distance, bearing):
    """ Returns the latitude and longitude of a location at a given distance
    and bearing from the original point using a local spherical approximation
    to the ellipsoid """
